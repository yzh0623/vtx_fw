 ...
 * @All right reserved.
 * @Project: vtx_slimopt
 * @File: UpdateRouter.java
 * @ClassName: UpdateRouter
 * @Description:更新路由
 *
 * @Author: yuanzhenhui
 * @Date: 2023/10/16
 */
public class UpdateRouter extends AbstractVerticle implements RouterSet {

    /**
     * eventbus访问路径
     */
    private static final String EVENTBUS_PUT_UPDATE =
        DaoConstants.UPDATE_DAO_FULL_NAME + RouterConstants.DOT + RouterConstants.UPDATE_BY_CONDITION;
    private static final String EVENTBUS_PUT_UPDATE_WITH_PK =
        DaoConstants.UPDATE_DAO_FULL_NAME + RouterConstants.DOT + RouterConstants.UPDATE_BY_PK;
    ...
    /**
     * 
     * @MethodName: putUpdate
     * @Description: 通过put方式进行条件更新
     * @author yuanzhenhui
     * @param ctx
     *            void
     * @date 2023-10-19 02:30:29
     */
    public void putUpdate(RoutingContext ctx) {
        ...
        if (null != clazz) {
            ...
            evbVtx.eventBus().request(EVENTBUS_PUT_UPDATE, jsonObj, RouterConstants.DELIVERY_OPTIONS,
    ...
    /**
     * 
     * @MethodName: putUpdateWithPk
     * @Description: 通过put方式进行主键更新
     * @author yuanzhenhui
     * @param ctx
     *            void
     * @date 2023-10-19 02:30:37
     */
    public void putUpdateWithPk(RoutingContext ctx) {
        HttpServerResponse hsr =
            ctx.response().putHeader(HttpConstants.HTTP_CONTENT_TYPE_PARAMNAME, HttpConstants.HTTP_CONTENT_TYPE);
        Class<?> clazz = RouterUtil.extractEntity(ctx.normalizedPath());
        if (null != clazz) {
            ...
            jsonObj.put(DaoConstants.CLAZZ_PATH, clazz.getSimpleName());
            evbVtx.eventBus().request(EVENTBUS_PUT_UPDATE_WITH_PK, jsonObj, RouterConstants.DELIVERY_OPTIONS,
                reMsg -> hsr.end(reMsg.result().body().toString()));
        } else {
            hsr.end(Json.encode(new RouterValue(0, RouterConstants.CAN_NOT_FIND_ENTITY)));
        }
    ...
    public void restRouter(Router router) throws ClassNotFoundException, IOException {
        ...
        if (null != clazzSet && !clazzSet.isEmpty()) {
            ...
            clazzSet.forEach(clazz -> {
                ...
                // 根据主键更新
                router.put(updateUri + pkParamUri).handler(this::putUpdateWithPk);
                // 根据条件更新
                router.put(updateUri + conditionParamUri).handler(this::putUpdate);
            });
        }
    }
}
